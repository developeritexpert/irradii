        ...
        $(el).editable({
            success: function (response, newValue) {
                ...
                if (response && response.new_id) {
                    ...
                    $(cloneAddElement).html('Add').css('color', '#a90329');
...
$(document).ready(function () {
    ...
    // 1. EDITABLES - Move to Top so they initialize even if other things fail
    ...
    // 2. Google Maps Autocomplete - Add Safety Checks
    var searchFldElement = document.getElementById('search-fld');
    if (typeof google !== 'undefined' && google.maps && google.maps.places && searchFldElement) {
        PageSearchesAlertsHelper.search_fld = new google.maps.places.Autocomplete(searchFldElement, { types: ['geocode'] });
    ...
    $('.dialog_delete_link').click(function () {
        if ($.ui && $.ui.dialog && $("#dialog_deleteItem").length) {
